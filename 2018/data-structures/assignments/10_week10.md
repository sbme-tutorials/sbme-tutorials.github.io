...
<img src="/gallery/Cmake.svg" style="width:60%;">
...
1. `cmake_minimum_required` to specify a minumum *cmake* version to run this script. This is used to maintain compatibility.
1. setting some first-class-citezen variable to declare our usage of the modern C++11 standard. The `set` takes two parameters seperated by a space. The first parameter is the variable name, while the second parameter is the value of that variable.
1. `project` here we give a name to our project.
...
1. `include_directory` cmake function takes multiple parameters. Here we introduced only `include` as a parameter. This command allows the project C++ source files to include any header file in the `include` directory. However, the source files and header files are still seperated and living in different locations.
1. `add_executable` cmake function takes two parameters: the executable (target) name, and the corresponding `.cpp` file.
